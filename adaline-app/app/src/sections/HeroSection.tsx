...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (heroContentRef.current) {
        ...
        if (heading) (heading as HTMLElement).style.transform = `scale(${scale})`;
        if (logos) (logos as HTMLElement).style.transform = `scale(${scale})`;
      ...
      if (contentSectionRef.current) {
        if (progress <= 0.20) {
          ...
          contentSectionRef.current.style.transform = `translateY(-${moveProgress * 100}vh)`;
  ...
  return (
    <>
      ...
      <div
        ref={heroContentRef}
        className="fixed top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center z-20 w-full max-w-4xl px-10 opacity-100 transition-opacity duration-300 pointer-events-none"
        style={{ willChange: 'opacity, transform' }}
      >
        <h1 className="text-4xl font-light text-white mb-6 leading-tight drop-shadow-lg">
          The single platform to iterate,<br />
          evaluate, deploy, and monitor AI agents
        </h1>
        <div className="mb-6">
          <p className="text-xs text-white uppercase tracking-widest mb-6 font-medium drop-shadow">
            Trusted by
          </p>
          <div className="trusted-logos flex justify-center items-center gap-16 overflow-hidden relative h-10">
            <div className="logos-container flex gap-16 animate-scroll-logos">
              {['Discord', 'McKinsey & Company', 'DOORDASH', 'HubSpot', 'Discord', 'McKinsey & Company', 'DOORDASH', 'HubSpot'].map((logo, i) => (
                <div key={i} className="logo-item text-xs text-white font-medium flex items-center gap-2 whitespace-nowrap drop-shadow">
                  {logo === 'Discord' && (
